...
files.forEach(file => {
  ...
  content = content.replace(/<(h[12])\b[^>]*>/gi, (match, tag) => {
    let newMatch = match;
    ...
    // clean up empty style blocks or trailing commas if any, but since we just removed keys, it might leave { style={{  fontWeight: 700 }}
